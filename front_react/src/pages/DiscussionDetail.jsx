    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          setDiscussion(d);
          setBuildingId(d.buildingId || null);
        }
      } catch (err) {
        console.error(err);
        setDiscussion(null);
...
function DiscussionHeader({ discussion, onChange }) {
  const auth = getAuth();
    ...
    <section className="card" id="discussionCard">
      ...
      <div className="meta muted" id="meta" style={{ marginTop: 4 }}>
        <span className="badge">
          {(discussion.status || "open").toUpperCase()}
        </span>{" "}
        {isPrivate ? (
        ...
        <span className="muted">
          Inicijator:{" "}
          {escapeHtml(discussion.ownerName || "(nepoznat)")}
        </span>{" "}
...
function DiscussionMessages({ discussion }) {
  ...
  useEffect(() => {
    ...
    load();
  }, [discussion.id, discussion.canViewContent, discussion.status]);
    ...
    <section className="card">
      ...
      <div className="messages" id="messages">
        ...
        {!loading &&
          ...
          messages.map((m) => {
            const author =
              m.authorName || m.authorEmail || m.authorId || "—";
            return (
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (!res.ok) {
          setSummary({ error: data.error || "Greška pri dohvaćanju glasanja." });
        } else {
          setSummary(data);
        }
      } catch (err) {
