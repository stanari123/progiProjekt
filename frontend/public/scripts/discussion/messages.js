window.discussionMessagesLoad = async function discussionMessagesLoad() {
  ...
  try {
    const res = await fetch(
      `${window.API_BASE}/discussions/${s.id}/messages`,
      {
    ...
    if (!res.ok) {
      ...
      msgEmpty && ((msgEmpty.style.display = ""), (msgEmpty.textContent = data.error || "Nema poruka ili nemate pristup."));
      return;
    }

    if (!Array.isArray(data) || data.length === 0) {
      messagesEl && (messagesEl.innerHTML = "");
      msgEmpty && ((msgEmpty.style.display = ""), (msgEmpty.textContent = "Nema poruka."));
      return;
    }
    ...
    console.error(err);
    messagesEl && (messagesEl.innerHTML = "");
    msgEmpty && ((msgEmpty.style.display = ""), (msgEmpty.textContent = "Greška u komunikaciji."));
  }
};
