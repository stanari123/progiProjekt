window.discussionVotesLoad = async function discussionVotesLoad() {
  ...
  try {
    const res = await fetch(
      `${window.API_BASE}/discussions/${s.id}/votes/summary`,
      {
    ...
    if (!res.ok) {
      voteInfo && (voteInfo.textContent = summary.error || "Greška pri dohvaćanju glasanja.");
      setBtns(false);
      return;
    }

    yesCount && ((yesCount.style.display = ""), (yesCount.textContent = `DA: ${summary.yes || 0}`));
    noCount && ((noCount.style.display = ""), (noCount.textContent = `NE: ${summary.no || 0}`));
    totalVotes && ((totalVotes.style.display = ""), (totalVotes.textContent = `Glasovi: ${summary.total || 0}`));
    thresholdEl &&
    ...
    if (closed) {
      voteInfo && (voteInfo.textContent = "Anketa je aktivna, ali rasprava je zatvorena.");
      setBtns(false);
      return;
    }
    ...
    voteYesBtn && voteYesBtn.classList.toggle("active", summary.currentUserVote === "yes");
    voteNoBtn && voteNoBtn.classList.toggle("active", summary.currentUserVote === "no");
