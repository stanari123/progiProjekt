window.discussionPollRender = function discussionPollRender() {
  ...
  if (pollActive) {
    ...
    pollDeleteBtn && (pollDeleteBtn.style.display = !discussionClosed && s.canModerate ? "" : "none");
    pollCreateBox && (pollCreateBox.style.display = "none");
    ...
    if (!discussionClosed && s.canModerate) {
      ...
      voteInfo && (voteInfo.textContent = "Nema aktivne ankete. Postavite pitanje.");
    } else {
      pollCreateBox && (pollCreateBox.style.display = "none");
...
(function bindPollActions() {
  ...
  pollCreateBtn?.addEventListener("click", async () => {
    ...
    const res = await fetch(
      `${window.API_BASE}/discussions/${s.id}/poll`,
      {
        method: "POST",
        headers: {
